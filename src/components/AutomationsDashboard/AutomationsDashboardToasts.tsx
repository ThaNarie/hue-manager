import type { AutomationsToast } from "./AutomationsDashboard.types";

type AutomationsDashboardToastsProps = {
  toasts: AutomationsToast[];
  onDismissToast: (toastId: string) => void;
};

export function AutomationsDashboardToasts({
  toasts,
  onDismissToast,
}: AutomationsDashboardToastsProps) {
  if (toasts.length === 0) {
    return null;
  }
