...
type ScenesDashboardListProps = {
  ...
  onBulkDeleteScenes: (sceneIds: string[]) => void;
};
...
export function ScenesDashboardList({
  ...
  const allScenesSelected =
    sceneIds.length > 0 && sceneIds.every((sceneId) => selectedSceneIds.includes(sceneId));

  useEffect(() => {
    setSelectedSceneIds((current) => current.filter((sceneId) => sceneIds.includes(sceneId)));
  }, [sceneIds]);
    ...
    <div className="space-y-2">
      <div className="space-y-2 rounded-md border border-border/70 bg-card/60 p-3">
        ...
        <div className="flex flex-wrap items-end gap-2">
          <label className="min-w-48 flex-1 space-y-1 text-xs text-red-100">
            <span>
              Destructive bulk delete. Type{" "}
              <span className="font-semibold">{DELETE_CONFIRMATION_TEXT}</span>.
            </span>
            <input
          ...
          <button
            type="button"
            className="h-8 rounded-md border border-red-500/40 px-3 text-xs font-medium text-red-100 transition hover:border-red-400 disabled:cursor-not-allowed disabled:opacity-60"
            onClick={() => {
              onBulkDeleteScenes(selectedSceneIds);
              setSelectedSceneIds([]);
                ...
                <div className="flex items-start gap-2">
                  ...
                  <div className="space-y-1">
                    ...
                    <p className="text-xs text-muted-foreground">
                      ID: {scene.id} · Last updated:{" "}
                      {new Date(scene.lastUpdatedAt).toLocaleString()}
                    </p>
                  </div>
                </div>
