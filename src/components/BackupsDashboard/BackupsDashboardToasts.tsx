import type { BackupToast } from "./BackupsDashboard.types";

type BackupsDashboardToastsProps = {
  toasts: BackupToast[];
  onDismissToast: (toastId: string) => void;
};

export function BackupsDashboardToasts({ toasts, onDismissToast }: BackupsDashboardToastsProps) {
  if (toasts.length === 0) {
    return null;
  }
