...
const DEFAULT_IDLE_BACKOFF_MULTIPLIER = 5;
...
function validateIdleBackoffMax(
  ...
  if (value === undefined) {
    return loopIntervalMs * DEFAULT_IDLE_BACKOFF_MULTIPLIER;
  }

  if (typeof value !== "number" || !Number.isInteger(value) || value <= 0) {
    throw new Error(
      `Invalid "idleBackoffMaxMs" in ${configPath}. Use a positive integer number of milliseconds.`,
    );
  }

  if (value < loopIntervalMs) {
    throw new Error(
      `Invalid "idleBackoffMaxMs" in ${configPath}. It must be greater than or equal to loopIntervalMs.`,
    );
  }
...
function validateIdleBackoffJitter(value: unknown, configPath: string): number {
  ...
  if (typeof value !== "number" || !Number.isInteger(value) || value < 0) {
    throw new Error(
      `Invalid "idleBackoffJitterMs" in ${configPath}. Use a non-negative integer number of milliseconds.`,
    );
  }
